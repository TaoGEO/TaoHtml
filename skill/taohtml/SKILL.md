...
Start every new invocation with a route handshake. The current message may establish
the route only when it contains a specific report topic or a source that is explicitly
bound to this task. Low-information text attached only so a platform can send the
Skill does not establish a route. Do not classify it with a list of forbidden words,
and do not scan the workspace for a presumed input.
...
8. Read `references/production-authorization.md`. Before confirmation, save each applicable current material summary, VI board, and Report Design Brief as a customer-readable task-local gate artifact; record its safe relative path, current SHA-256, and confirmation trace. Run `scripts/check_production_authorization.py --artifact-root <current-task-root> --action formal-html`. Only a successful current-file check authorizes formal HTML; rerun it before browser QA and delivery so any post-confirmation byte change fails closed until reconfirmed. A bound-material summary preview, static-reference VI standards-board preview, and Report Design Brief remain allowed only in their matrix states; none is a formal report preview. After authorization, read `references/process-playbook.md` and produce the HTML directly. Do not insert a separate full prose manuscript step. Follow its **first runnable artifact** cadence: lock a concise page plan, save a complete runnable `index.html`, then refine and QA in bounded passes. On the static-reference route, explicitly load the compiled project theme through the shared renderer; never substitute one of the four built-in themes.
...
10. Run asset QA, then run the `browser` profile before browser QA. Check every controlled reveal state for slide bounds, text collisions, and readable content clipped by any ancestor whose computed overflow axis is `hidden`, `clip`, `auto`, or `scroll`. Fix objective failures and report the files, checks, and production-stage judgment calls. Do not claim zero overflow or completion while browser QA fails. Deliver the usable report together with the concise `《待核实内容清单》` defined in `references/process-playbook.md`; do not withhold the report merely because ordinary creative supplements still await customer verification.
...
- For every material actually used, record its path or upload identity, `source_binding`, and binding reason in the material summary and Report Design Brief. Do not read an unbound candidate merely to decide whether it is useful.
...
- `scripts/preflight.py`: run `core`, `pdf`, `static-reference`, or `browser` capability checks before the corresponding work. Its parent process uses only the Python standard library.
- `scripts/check_production_authorization.py`: validate current material-summary, VI, project-theme, and design-brief state before a requested preview, formal HTML, QA, or delivery action.
- `scripts/check_assets.py`: find missing, remote, or non-portable assets.
- `scripts/check_html_deck.py`: exercise routes, every reveal state, runtime behavior, media, console errors, slide bounds, text collisions, ancestor clipping of readable content, and screenshots.
...
- Check console errors, visible slide bounds, and ancestor clipping of readable content at every controlled reveal state and target viewport. An element remaining inside the slide does not pass when an intermediate clipping ancestor hides it.
- In `corporate_fidelity`, require both fixed-layer fidelity and complete readability inside the selected editable region. Do not weaken the fixed brand boundary to make an overloaded page appear to pass.
