...
5. After content and structure are clear, resolve the visual source. If the user chooses “use my reference” and supplies exactly one static image, read `references/static-reference-vi.md`, pass its current-session readability gate, analyze static design language only, render a VI design standards board, and wait for the current board's explicit “确认 VI”. Do not infer dynamic rules, force an internal theme, create a prose-only visual summary, begin project-theme generation, or start formal report production before this confirmation. If no clear reference exists, read `references/visual-systems.md`, recommend 2-3 suitable built-in systems with name, description, and preview, then accept the user's selection or delegation. Stay inside the existing clarification-question budget; when the budget is exhausted or the user delegates, select one and disclose the choice.
6. After the applicable source gate passes, any single-static-reference VI board is confirmed, and no high-risk gap remains, read `references/design-brief-template.md` and produce one adaptive Report Design Brief. Record the visual source, confirmed VI board when applicable, selected built-in system when applicable, and any necessary deviation. For a conversion goal, record the desired action, exact verified path, provenance and verification status, and final display method. Put every safe inference in its confirmation section. VI confirmation is not Report Design Brief confirmation.
...
8. After confirmation, read `references/process-playbook.md` and produce the HTML directly. Do not insert a separate full prose manuscript step. Follow its **first runnable artifact** cadence: lock a concise page plan, save a complete runnable `index.html`, then refine and QA in bounded passes. On the single-static-reference route, also require a compatible project-specific theme output before production; this v1 workflow defines the confirmed-VI handoff but does not compile that theme or substitute one of the four built-in themes.
...
- `references/static-reference-vi.md`: single-static-reference readability gate, observed/extension/unknown boundary, VI board contract, confirmation gate, and next-task handoff.
