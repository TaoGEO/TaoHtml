...
10. Run asset QA, then run the `browser` profile before browser QA. Fix objective failures and report the files, checks, and production-stage judgment calls. Deliver the usable report together with the concise `《待核实内容清单》` defined in `references/process-playbook.md`; do not withhold the report merely because ordinary creative supplements still await customer verification.
