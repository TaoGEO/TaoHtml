...
5. After content and structure are clear, resolve the visual source. If the user chooses “use my reference” and supplies exactly one static PNG, JPEG, or WebP screenshot, identify `reference_mode`. When intent is still unclear, ask exactly one binary question: **参考风格重构** (extract the design language and allow recomposition) or **企业模板保真** (lock the screenshot's visible Logo, header, footer, brand bars, and fixed decoration; design only the safe content region). Count this question inside the existing clarification budget. If the user already says “企业模板保真”, “公司模板原样采用”, or an equivalent clear instruction, set `corporate_fidelity` and do not ask again; otherwise preserve the existing `reconstruct` behavior. Read `references/static-reference-vi.md`, pass its current-session readability gate, analyze the still only, fill the shared executable-layout grammar and mode-specific fields, render one VI design standards board, and wait for the current board's explicit “确认 VI”. Both modes use this same gate. Do not infer dynamic rules, force an internal theme, create a prose-only visual summary, begin project-theme generation, or start formal report production before this confirmation. If a clear reference is a PPT, webpage, video, multiple images, or a state sequence, treat it as unsupported in v1 and ask for one representative static screenshot; do not infer movement or route it as “no reference.” Only when no clear reference exists, read `references/visual-systems.md`, recommend 2-3 suitable built-in systems with name, description, and preview, then accept the user's selection or delegation.
6. After “确认 VI” on either single-static-reference mode, read `references/project-theme-compiler.md`, create its hash-bound handoff, and compile the current project's theme before the Report Design Brief. In `corporate_fidelity`, bind the source-image hash and dimensions, normalized locked/editable regions, extraction records, extension pages, and limitations; deterministically crop and embed fixed elements without model redraw, keep them stationary, and confine layout and Runtime reveal behavior to the editable region. Never use the full screenshot as a page background. Do not add the result to the four built-in systems, compile an unconfirmed or changed VI, or infer motion from the still. On the no-reference route, keep the selected built-in system instead. Then, after the applicable source gate passes and no hard-boundary gap remains, read `references/design-brief-template.md` and produce one adaptive Report Design Brief. Record `reference_mode`, the fidelity boundary, locked elements, editable region, observed/extension/unknown boundary, confirmed VI and compiled project theme when applicable, selected built-in system when applicable, any necessary deviation, and the planned creative-supplement scope. VI confirmation is not Report Design Brief confirmation.
...
- In `corporate_fidelity`, preserve only screenshot-visible fidelity. Do not promise recovery of the original PPT master, vector Logo, font files, or unseen assets. Never ask a model to redraw a Logo; request a clearer screenshot or independent Logo when deterministic extraction is unreliable.
...
- `references/static-reference-vi.md`: one-time reconstruct/corporate-fidelity routing, single-static-reference readability gate, observed/extension/unknown boundary, exact executable layout grammar, corporate locked/editable-region contract, VI board, confirmation gate, and next-task handoff.
- `references/project-theme-compiler.md`: post-“确认 VI” hash-bound handoff, deterministic structural project-theme compilation, corporate crop/shell generation, eligible/compiled usage mapping, fallback policy, explicit project rendering, and verification.
...
- `scripts/render_reference_vi.py`: validate legacy reconstruct v1.1 or the shared v1.2 reference contract, verify corporate source hash/dimensions and normalized regions, extract exact fixed-element crops, render the standards-board HTML, and export a 3200×2400 PNG. It does not analyze the image or compile a project theme.
- `scripts/compile_project_theme.py`: validate the exact confirmed-VI handoff and shared executable-layout compatibility matrix, then deterministically write project-local manifest, CSS, templates, and provenance. Corporate mode embeds only hashed fixed-element crops, emits one reusable stationary shell, and wraps all report content in the editable region. It does not call a model or add a built-in theme.
