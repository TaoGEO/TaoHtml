...
If the current session cannot reliably locate static visual facts, follow the two recovery paths in `static-reference-vi.md`; do not guess or maintain a model matrix here. Dynamic references require a future workflow and must not be represented as supported by the single-static-reference route.
