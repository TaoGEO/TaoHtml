...
For `continue_existing`, use this reference only for the affected source delta; do
not restart full material intake when the inherited interpretation remains supported.
...
coverage, what it supports, and what it cannot establish. Use the exact role and
status vocabulary in `project-handoff.md`. `workspace_readable` describes current
access, not provenance; record whether inspection was complete or partial.
...
For continuation, keep the summary compact and delta-oriented: identify which prior
interpretation remains supported, what the requested change affects, and which source
roles or availability gaps constrain that change. Do not imply that a detailed
secondary summary is sufficient for every later edit.
...
During continuation, layout, technical, portability, and meaning-preserving local
expression changes may proceed without restoring every unavailable original. Before
changing real data, source attribution, evidence relationships, real identities,
achieved outcomes, or core conclusions, restore the original source or obtain the
user's explicit confirmation of the exact change as defined in
`project-handoff.md`. High-risk factual verification remains mandatory.
