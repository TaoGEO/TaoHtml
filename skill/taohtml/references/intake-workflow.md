...
- Only when no enterprise profile applies and no clear reference route is active, read `visual-systems.md`. If the user already specified one concrete built-in system, adopt it directly without displaying the catalog. If the user semantically narrowed the acceptable range, show only the matching business-oriented or design-led subset; infer the scope from meaning and constraints rather than literal phrase matching. Otherwise, in the same round, show every system in the complete current built-in catalog with its exact customer-facing name, one-line description, and bundled preview. Do not hide any entry because the Agent considers it less suitable. The Agent may mark one or two displayed systems as **更推荐** and explain the report-goal, audience, content, and use-mode basis, but recommendation never replaces complete catalog display. Ask the user to choose once, or invite explicit delegation to TaoHtml.
