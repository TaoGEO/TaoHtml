# 产品介绍

## 一句话定位

TaoHtml 是一个面向 AI Agent 的高设计 HTML 课件生成 skill。

它帮助 Codex 将 PDF、截图、文档、汇报材料、课程大纲、已有课件，重构成可演示、可追溯、可打包交付的 HTML Presentation。

English brief: TaoHtml helps AI agents turn source documents into high-design, source-traceable HTML presentations.

## 产品承诺

TaoHtml 不只是“把文件转成网页”。

它帮助 Agent 做四个关键判断：

1. 这份课件要让观众形成什么判断？
2. 哪些原始信息、证据、图表和数字必须保留？
3. 哪些页面需要为了现场演讲重新设计？
4. 最终文件如何检查、打包，并在另一台电脑上稳定打开？

## 用户痛点

AI 很容易生成页面，但经常出现这些问题：

- 页面变成“标题 + 卡片”的重复堆砌。
- 原 PDF 只是被截图塞进页面，没有演讲逻辑。
- 图表、数据、截图太小，观众看不清。
- 换了配色，但构图依然普通。
- 有动画，但动画不服务口播节奏。
- 在当前电脑能打开，换一台电脑就丢图、丢视频、路径失效。

TaoHtml 的价值，是把这些失败模式变成明确的流程检查和交付标准。

## TaoHtml 的方法

TaoHtml 把课件看成一条可控的说服序列：

1. 重新定义受众正在面对的问题。
2. 用真实证据说明问题正在发生。
3. 解释问题背后的机制。
4. 展示方法、系统或工具。
5. 用证据、边界和流程降低信任风险。
6. 用清晰下一步完成收口。

主页面负责现场说服，证据层负责信息保真。

## 真实案例

TaoHtml 的第一个标杆案例是一套 GEO 沙龙路演 HTML 课件。

这个案例从活动海报、课程框架、AI 问答截图、复采录屏、复采报告、任务盘和报价材料开始，最终被重构成一套可现场演讲的 HTML Presentation。它不是把资料直接排版，而是把材料组织成“客户行为变化 → AI 搜索机制 → GEO 四个关键词 → 真实证据与回测 → 系统化工程 → 服务转化”的完整演讲链路。

案例详情见：

- `docs/gallery.md`
- `docs/cases/geo-salon.md`

## 和普通 HTML 课件生成有什么不同

### 1. 先判断输出模式

TaoHtml 会区分两个任务：

- 忠实迁移：保留源文件页面，让它变得可浏览、可翻页、可打包。
- 路演重构：把源文件当作信息和证据，重新设计主页面。

这能避免一个常见错误：明明只是套了一个 PDF 浏览器，却声称完成了设计重构。

### 2. 内置高设计评分标准

TaoHtml 内置 `design-quality-rubric.md`。

它会从故事、证据、版式、动效、可迁移性、文字溢出等维度判断：这份课件到底是“普通汇报”，还是“真正设计过的演示稿”。

### 3. 内置版式母型库

TaoHtml 内置 12 类版式母型：

- 海报开场
- 左右冲突
- 证据舞台
- 机制流程
- 系统剖面
- 案例拆解
- 指标看板
- 诊断矩阵
- 现场演示壳
- 报价 / 方案页
- 原始页查看器
- 收尾页

这些母型帮助 Agent 避免把所有页面都做成同一种卡片布局。

### 4. 内置 HTML 交付纪律

TaoHtml 提供本地 HTML 模板和 QA 脚本，用于：

- 16:9 演示结构
- 键盘和点击翻页
- 分步展开
- 原始页弹窗
- 资产路径检查
- 浏览器截图
- QA 总览图
- zip 打包

## 目标用户

TaoHtml 适合：

- 准备路演的创业者
- 做客户提案的咨询顾问
- 制作课程课件的内容创作者
- 需要把 PDF 改成演示稿的业务团队
- 做证据型汇报的运营和项目负责人
- 需要稳定工作流的 AI Agent 使用者

## 产品边界

TaoHtml 不是完整的设计软件，也不是 PPTX 编辑器。

它是一个 Codex skill：由说明、参考文档、HTML 模板和脚本组成，帮助 AI 编程代理稳定地产出更好的 HTML 课件。

## 作者与合作

TaoHtml 由 Tao 发起。

可交流方向：

- 企业汇报 / 路演课件 HTML 化
- AI Agent skill 定制
- 高设计感商业报告与演示系统
- GEO / 内容工程 / 知识库相关报告系统

微信：`taomir`
